...
char value[2*720];//2*imageSize]; // Adjust the size to match the desired read length

// Reorganize the data as specified
static u32 *reorganized_value = (u32 *) value;
static u32 DMA_O_dest_str[720]; // u32 : 2*u8, 2*u8
...
//	Xil_ExceptionEnable();
	...
	Xil_DCacheFlushRange((u32)RX_buf, 720*4);
...
//			xil_printf("File Pointer: %ld\r\n", file_pointer);
			 ...
			 // Xil_DCacheFlushRange((u32)RX_buf, 720*4);	DMA_O_dest_str
			 XAxiDma_SimpleTransfer(&myDma2, (u32)RX_buf, 720*4, XAXIDMA_DEVICE_TO_DMA);
//			 if (status != XST_SUCCESS)
//				 xil_printf("DMA failed\n\r", status);

			 // Xil_DCacheFlushRange((u32)DMA_O_dest_str, 720*4);				//720*240
			XAxiDma_SimpleTransfer(&myDma, (u32)DMA_O_dest_str, 720*4, XAXIDMA_DMA_TO_DEVICE);//typecasting in C/C++
//			if (status != XST_SUCCESS)
