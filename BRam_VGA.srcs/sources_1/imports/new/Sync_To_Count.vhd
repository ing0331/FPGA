...
begin
  ...
  gen_sync: process(i_Clk)
  begin
    if TO_INTEGER(r_Row_Count) < g_ACTIVE_COLS + h_front_porch or TO_INTEGER(r_Col_Count) > g_TOTAL_COLS - h_back_porch - 1 then 
      i_HSync <= '1';
    else    
      i_HSync <= '0';
    end if;
    if TO_INTEGER(r_Row_Count) < g_ACTIVE_ROWS + v_front_porch or TO_INTEGER(r_Row_Count) > g_TOTAL_ROWS - v_back_porch - 1 then 
      i_VSync <= '1'; 
    else    
      i_VSync <= '0';
  end if;
